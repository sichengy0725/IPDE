draws_JAGS <- function(y, d, p, e,
  ...
  smp <- coda.samples(
    model          = jags,
    variable.names = c("beta0", "beta1", "b0", "b1", "b2"),
    n.iter         = n.iter,
    thin           = thin,
    progress.bar   = "none"
  )
...
futility <- function(draws, E.TARGET, E.cutoff, p){
  ...
  for(j in 1:length(p)){
    pi1_draws <- plogis(draws$b0 + draws$b1 * p[j] + draws$b2 * p[j]^2)
...
get.utility <- function(draws, uti, p){
  ...
  posteff <- vapply(seq_len(J), function(j) {
    mean(plogis(draws$b0 + draws$b1 * p[j] + draws$b2 * p[j]^2))
  }, numeric(1))
  postnotox <- 1 - posttox
  postnoeff <- 1 - posteff
  cat('posttox:', posttox, '\n')
...
#uti[4] - tox, no eff
uti = c(100,40,60,0)
ske1 = c(0.02, 0.06, 0.08, 0.1, 0.2,0.3)
#backsolve for d_j
dose = backsol(ske1, mu_beta0 = 3, mu_beta1 = 1)
T.PI = c(0.02, 0.05, 0.08, 0.1, 0.3,0.5)
E.PI = c(0.1,rep(0.4,5))
res <- IPCRM_TS(
    T.PI,
    E.PI,
    T.TARGET = 0.3,
